...
**受信 Codex エンドポイント。** shunt は逆方向にも動作します。オプトインの `[server.codex_endpoint]` テーブルは、生の OpenAI Responses HTTP/SSE と WebSocket トランスポート（`/responses`、`/v1/responses`、`/backend-api/codex/responses`）を登録するため、**Codex CLI 自体**が `base_url` を shunt に向け、同じ ChatGPT/Codex OAuth アカウントプールでロードバランスできます。一意な厳密一致の `[models.upstream_model]` または `[[routes]]` は互換 Responses プロバイダーを 1 つ選択できますが、プレフィックスのみ・非厳密・未一致のモデルは固定された `server.codex_endpoint.provider` へフォールバックします。厳密でも曖昧、変換専用/非 Responses、モデル書き換えの宣言はディスパッチ前に拒否されます。HTTP はバイト単位のリレーを維持し、WebSocket は有界イベントをストリーミングし、出力開始後のプロバイダー移行はありません。デフォルトでは無効で、このテーブルがなければルートは登録されません。[使い方](https://shunt.dev/ja/guides/inbound-codex-endpoint/)と [M11 動作仕様](docs/m11-inbound-codex-endpoint.md)を参照してください。
...
同じオプトイン面は HTTP 専用の `POST /v1/responses/compact` も登録します。本文と不透明な継続状態を、同じ認証、リクエスト上限、ネイティブルート判定、アカウントプールを通して、検証済みのネイティブ compact バックエンド（ChatGPT/Codex または公式 OpenAI API）へバイト単位でそのまま転送します。未対応、曖昧、変換が必要、または不正な対象はネットワーク送信前に拒否されます。shunt は継続状態を復号せず、要約を合成せず、リクエスト履歴を保存しません。
