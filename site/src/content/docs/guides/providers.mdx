...
- **`kind = "openai_chat"`** — the upstream speaks the OpenAI Chat Completions API. shunt translates Anthropic Messages ⇄ Chat Completions, including streaming; see the [OpenAI-compatible provider page](/providers/openai-chat/).
...
The opt-in `opencode-go` preset uses `kind = "opencode_go"`,
`SHUNT_OPENCODE_GO_API_KEY`, and the canonical destination
`https://opencode.ai/zen/go/v1`. OpenCode Go is **not supported**: the
empty admitted set permits no model. A Go primary rejects the whole request before credential lookup
or dispatch; Go fallback candidates are removed. Neither credentials nor
`x-opencode-session` are sent. See [OpenCode Go](/providers/opencode-go/) for
the candidate-only evidence ledger and strict terminal/promotion requirements.

## Command Code presets

`commandcode` is the API-key Chat product (`openai_chat`, `SHUNT_COMMANDCODE_API_KEY`); `command-code` is the separate subscription NDJSON product (`command_code`, `command_code_oauth`, `SHUNT_COMMAND_CODE_TOKEN`). Declare them explicitly in `[[upstreams]]`. Only an absent subscription token variable permits the fixed read-only `~/.commandcode/auth.json` fallback; empty/invalid values never fall back. There is no refresh or writeback. Exact model/effort rows, limits and the source-derived—not live-verified—evidence boundary are in [Command Code](/providers/command-code/).
...
`openai_chat` fallbacks support tools and base64/URL images, but are excluded for structured output or explicit reasoning effort. The configured primary is retained and validates unsupported input normally.
