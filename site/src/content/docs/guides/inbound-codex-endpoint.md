...
- **Exact native model routing.** A unique exact `[models.upstream_model]` or `[[routes]]` declaration may select one compatible Responses-native provider and forwards the original model/body unchanged. Prefix-only, non-exact, and unmatched models use the pinned `[server.codex_endpoint].provider`; exact ambiguous, translated, or incompatible declarations reject before any upstream request.
