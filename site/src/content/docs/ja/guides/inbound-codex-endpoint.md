...
- **厳密一致のネイティブルーティング。** 一意な厳密一致の `[models.upstream_model]` または `[[routes]]` 宣言は、互換性のある `kind = "responses"` プロバイダー 1 つを選択し、元の model/ボディを変更せず転送できます。プレフィックスのみ、非厳密一致、未一致のモデルは固定された `[server.codex_endpoint].provider` へフォールバックし、厳密でも曖昧、変換専用/非 Responses、またはモデルを書き換える宣言は上流リクエスト前に拒否されます。
