...
| `kind` | preset がない場合 | `anthropic`、`responses`、`openai_chat`、`command_code`、`cursor`、`gemini`、`antigravity`、`antigravity_cli`。後者 3 つは下記の preset 表に項目がないため（組み込みの `[providers.gemini]`、`[providers.antigravity]`、`[providers.antigravity-cli]` テーブルは preset ではなく、別建てのレガシー方式です）、順序付き upstream では `kind` を明示的に指定する必要があります。CLI provider のテーブル名はハイフンの `antigravity-cli` ですが、`kind` 値はアンダースコアの `antigravity_cli` です。`kind = "openai_chat"` は追加で `env`（レガシー形式では `api_key_env`）付きの `auth = "api_key"` が必要で、他の credential モードは受け付けません。  `command_code` は別のサブスクリプション NDJSON アダプターです。[Command Code](/ja/providers/command-code/)を参照。 |
...
`command_code_oauth` マップは `mode` のみを受け付け、アカウント・認証パス・バージョン上書きはありません。`SHUNT_COMMAND_CODE_TOKEN` が存在しない場合のみ `~/.commandcode/auth.json`（`apiKey`、任意の `userId`）を読み取り専用で使用します。空・不正な明示値は拒否します。ファイル上限は 16 KiB、取得待機は 5 秒で、更新・コピー・書き込みは行いません。正規 HTTPS ホスト/443 と空パス、`/`、`/alpha/generate` のみ許可し、ユーザー情報・クエリ・フラグメント・リダイレクトは禁止です。effort、応答上限、Phase 16 ライブ検証の注意点は [Command Code](/ja/providers/command-code/)を参照してください。
...
将来の昇格には正確なモデル・宛先・wire・effort・capability の証拠、hermetic 適合性、資格情報安全な検証が必要です。条件付きの `x-opencode-session` は一致する Chat 契約を再利用する場合に限り正規の宛先へ送信できます。現在の空の許可集合にはセッション生成器がありません。
