...
Endpoint routes are matched by exact, case-sensitive byte equality before the legacy global
`[models.upstream_model]`/`[[routes]]` resolver. Each route names a credentialed `kind = "responses"`
provider and may set `upstream_model` (defaulting to `model`). A non-match retains global exact
resolution, including `[1m]` normalization, then falls back to the pinned `provider`.

Registers `POST /backend-api/codex/responses`, `POST /responses`, and `POST /v1/responses`. Routing follows the precedence above. With `[server.auth]`, clients need a valid token; without it, reachable clients can use operator credentials, so protect access beyond loopback. Native Responses paths relay the response verbatim; global Anthropic mappings use the strict translation bridge.
...
Without an endpoint-local match, global routing retains its existing behavior: strip a trailing
`[1m]` hint, then check a unique exact `[models.upstream_model]` or legacy `[[routes]]` declaration.
Native Responses mappings cannot rewrite the model; exact Anthropic mappings use the strict
translation/collaboration bridge. Ambiguous maps and unsupported adapters reject before dispatch.
Unmatched models use the pinned provider. No provider hop occurs after output begins.

Endpoint-local route entries contain `model` (required public id), `provider` (required configured
Responses provider), and optional `upstream_model` (defaults to `model`). Credential-free modes,
duplicate ids, blank fields and non-Responses providers are rejected. The upstream M16
Anthropic/Chat modules remain translation-core only, not additional endpoint-local targets.
