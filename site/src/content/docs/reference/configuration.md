...
| `kind` | without a preset | `anthropic`, `responses`, `openai_chat`, `cursor`, `gemini`, `antigravity`, or `antigravity_cli`. `openai_chat` and the last three have no entry in the preset table below — the built-in `[providers.gemini]`, `[providers.antigravity]`, and `[providers.antigravity-cli]` tables are the separate legacy mechanism, not presets — so an ordered upstream on any of them must set `kind` explicitly. Note that the CLI provider's table is named `antigravity-cli` with a hyphen, while its `kind` value is `antigravity_cli` with an underscore. `kind = "openai_chat"` additionally requires `auth = "api_key"` with an `env` (`api_key_env` in the legacy form); no other credential mode is accepted. |
