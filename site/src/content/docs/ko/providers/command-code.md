...
허용되는 최상위 필드는 `model`, `messages`, `max_tokens`, `stream`, `system`, `output_config`, `temperature`, `tools`, `tool_choice`입니다. `top_p`, `top_k`, `stop_sequences`, 최상위 `thinking`, `metadata`를 포함한 다른 필드는 400을 반환하며, 조용히 무시하거나 전달하지 않습니다. `output_config.effort`에는 정확히 지원되는 값을 사용하세요.

도구 목록은 이름순으로 정렬됩니다. `tool_choice: any`는 적어도 한 번의 도구 호출을 요청하는 시스템 지시를 추가합니다. `tool_choice: tool`은 목록을 선택한 도구로 제한하고 해당 도구를 명시하는 지시를 추가합니다. 이는 모델에 보이는 지시이며, 도구 호출을 보장하는 네이티브 프로토콜 기능이 아닙니다. `auto`는 지시를 추가하지 않고 `none`은 목록을 제거합니다.
