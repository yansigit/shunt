...
- **손실성 입력은 전송 전에 실패합니다.** `previous_response_id`, 암호화 reasoning/compaction 상태, hosted/custom 도구, 원격 파일 id, 잘못된 도구 관계와 미지원 필드는 추측하지 않고 거부합니다. 네이티브 Responses와 네이티브 compaction에는 영향이 없습니다.
...
- **네이티브 compaction은 불투명하게 유지됩니다.** HTTP 전용 `POST /v1/responses/compact`는 동일한 인증, 요청 제한, 네이티브 라우팅 및 계정 풀을 사용하며 본문과 연속 상태를 검증된 ChatGPT/Codex 또는 정식 OpenAI compact 엔드포인트로 바이트 단위 그대로 전달합니다. 잘못되었거나 모호하거나 변환이 필요하거나 지원되지 않는 대상은 네트워크 디스패치 전에 거부됩니다. shunt는 연속 상태를 복호화하거나 요약을 합성하거나 요청 기록을 저장하지 않습니다.
