...
옵트인 `opencode-go` 프리셋은 `kind = "opencode_go"`,
`SHUNT_OPENCODE_GO_API_KEY`, 표준 대상 `https://opencode.ai/zen/go/v1`을
사용합니다. OpenCode Go는 현재 지원되지 않습니다. 허용 집합은 비어 있으며
허용된 모델이 없습니다. Go가 기본 대상이면 자격 증명 조회나 전송 전에 요청
전체를 거부하고, 후속 Go 후보는 제거합니다. 자격 증명과 `x-opencode-session`은
전송하지 않습니다. 후보 전용 증거와 엄격한 종료·승격 요건은
[OpenCode Go](/ko/providers/opencode-go/)를 참조하세요.

## Command Code 프리셋

`commandcode`는 API 키 Chat 제품(`openai_chat`, `SHUNT_COMMANDCODE_API_KEY`)이고, `command-code`는 별도 구독 NDJSON 제품(`command_code`, `command_code_oauth`, `SHUNT_COMMAND_CODE_TOKEN`)입니다. `[[upstreams]]`에 직접 선언하세요. 구독 토큰 변수가 아예 없을 때만 고정 `~/.commandcode/auth.json`을 읽기 전용으로 사용하며, 빈 값·잘못된 값은 대체하지 않습니다. 갱신·쓰기는 없습니다. 정확한 모델·노력 수준, 한도, 실서비스 검증이 아닌 소스 기반 증거 범위는 [Command Code](/ko/providers/command-code/)를 참조하세요.
