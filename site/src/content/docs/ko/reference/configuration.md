...
| `kind` | preset이 없는 경우 | `anthropic`, `responses`, `openai_chat`, `cursor`, `gemini`, `antigravity`, `antigravity_cli` 중 하나. `openai_chat`과 뒤의 세 종류는 아래 preset 표에 항목이 없으므로 — 내장 `[providers.gemini]`, `[providers.antigravity]`, `[providers.antigravity-cli]` 테이블은 preset이 아니라 별개의 레거시 방식입니다 — 정렬 업스트림에서 `kind`를 직접 지정해야 합니다. CLI provider의 테이블 이름은 하이픈을 쓰는 `antigravity-cli`이지만, `kind` 값은 밑줄을 쓰는 `antigravity_cli`입니다. `kind = "openai_chat"`은 추가로 `env`(레거시 형식에서는 `api_key_env`)가 있는 `auth = "api_key"`가 필요하며, 다른 credential 모드는 허용되지 않습니다. |
