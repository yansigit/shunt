...
향후 승격에는 정확한 모델·대상·wire·effort·capability 증거, hermetic 적합성 및 자격 증명 안전 검증이 필요합니다. 조건부 `x-opencode-session`은 일치하는 Chat 계약을 재사용할 때 표준 대상에만 보낼 수 있습니다. 현재 빈 허용 집합에서는 세션 생산자가 없습니다.
