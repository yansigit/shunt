...
JSON and SSE preserve reasoning/text block order and final usage. JSON
accumulation is bounded by a conservative 64 MiB allocation budget. HTTP error
diagnostics retain at most 64 KiB of body bytes, with a five-second read deadline. Cancelling
before headers or during either response mode closes the upstream turn and
releases gateway capacity; repeated text never substitutes for a terminal.

Cursor does not automatically retry a Run. A configured fallback can be used
only after a proven connection failure before sending. Accepted HTTP errors,
ambiguous post-send timeouts, partial output, and tool calls never trigger replay.

Malformed END JSON, gzip, protobuf, or MCP argument values return errors rather
than successful completion or substituted values. Valid empty/gzipped END
trailers and well-formed unknown protobuf fields remain supported.

Idle timeouts and duplicate/post-terminal bytes in an already-received batch are errors, not successful completion. Run output-token deltas are cumulative in streamed usage updates; context checkpoints replace the previous absolute occupancy rather than being added. `usage.estimated: true` labels the derived input count. Zero input/cache values (and zero output without deltas) are unavailable-data placeholders, not measured usage or billing figures.
