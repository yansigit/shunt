...
可选的 `opencode-go` 预设使用 `kind = "opencode_go"`、
`SHUNT_OPENCODE_GO_API_KEY` 和规范目标 `https://opencode.ai/zen/go/v1`。
OpenCode Go 目前不受支持，准入集合为空，没有已准入模型。Go 作为主目标时，
会在凭据查找或发送前拒绝整个请求；后续 Go 候选会被移除。
当前不会发送凭据或 `x-opencode-session`。候选证据及严格终止和准入要求见
