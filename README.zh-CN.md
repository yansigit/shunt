...
**入站 Codex 端点。** shunt 也可以反向运行:可选的 `[server.codex_endpoint]` 表会注册原始 OpenAI Responses HTTP/SSE 和 WebSocket 传输(`/responses`、`/v1/responses`、`/backend-api/codex/responses`),使 **Codex CLI 本身**可以把 `base_url` 指向 shunt,并在同一个 ChatGPT/Codex OAuth 账号池上做负载均衡。HTTP 保持逐字节中继;WebSocket 在返回 `101` 前完成认证,在本地完成 `generate: false` 预热,并以有界方式流式传输 Responses 事件,在请求替换或连接断开时取消上游工作。它还把 CLI 的两个分析路径作为保护隐私的丢弃接收端。该功能默认关闭;没有此表时不会注册这些路由。请参阅[使用方法](https://shunt.dev/zh-cn/guides/inbound-codex-endpoint/)和 [M11 行为规范](docs/m11-inbound-codex-endpoint.md)。
