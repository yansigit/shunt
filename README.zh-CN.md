...
**入站 Codex 端点。** shunt 也可以反向运行:可选的 `[server.codex_endpoint]` 表会注册原始 OpenAI Responses HTTP/SSE 和 WebSocket 传输(`/responses`、`/v1/responses`、`/backend-api/codex/responses`),使 **Codex CLI 本身**可以把 `base_url` 指向 shunt,并在同一个 ChatGPT/Codex OAuth 账号池上做负载均衡。唯一的精确 `[models.upstream_model]` 或 `[[routes]]` 声明可以选择一个兼容的 Responses 提供方;仅前缀、非精确和无匹配模型回退到固定的 `server.codex_endpoint.provider`。精确但有歧义、需要转换/非 Responses 或改写模型的声明会在分发前拒绝。HTTP 保持逐字节中继;WebSocket 以有界方式流式传输 Responses 事件;输出开始后不会切换提供方。该功能默认关闭;没有此表时不会注册这些路由。请参阅[使用方法](https://shunt.dev/zh-cn/guides/inbound-codex-endpoint/)和 [M11 行为规范](docs/m11-inbound-codex-endpoint.md)。
