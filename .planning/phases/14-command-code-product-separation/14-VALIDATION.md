...
| --- | --- | --- | --- | --- | --- | --- |
| API-key preset + siblings | 14-01 Task 1 | CCK-01 | commandcode preset over openai_chat, sibling rows untouched, distinct env name | cargo test --test command_code_api_conformance command_code_api_preset | unit/config | pending |
| API-key tracer + isolation | 14-01 Task 2 | CCK-01/02, D-12/13 | real-router unary, concurrent no-crossover, auth error no-retry; RED recorded before GREEN | cargo test --test command_code_api_conformance command_code_api_tracer | real gateway | pending |
| API-key scenarios | 14-01 Task 3 | CCK-03, D-11 | unary/stream/tools/long-context/drop through actual gateway | cargo test --test command_code_api_conformance command_code_api_scenarios | real gateway | pending |
| Protocol evidence gate | 14-02 Task 0 | D-12 | dated ledger, three sections, pinned revision 055c3ecf, MIT attribution, finish grammar | test -s + rg section/revision checks | artifact gate | pending |
| Subscription tracer + destination | 14-02 Task 1 | CCS-02, D-01..04, D-08 | /alpha/generate canonical, bearer env-first, destination-before-credential + at dispatch, single terminal, EOF fail-closed; RED recorded | cargo test --lib command_code_tracer | real gateway | pending |
| Env credential matrix | 14-02 Task 2 | CCS-01, D-03 | valid/invalid/absent/empty env; fail-closed; snapshots immutable; file unchanged | cargo test --lib command_code_tracer_env_matrix | resolver + fixture files | pending |
| Effort table + subscription preset | 14-03 Task 1 | CCS-03, D-05 | exact pinned arrays, ultra rejected, no clamp/alias, sibling preservation | cargo test --test command_code_translate command_code_translate_effort; ... command_code_translate_preset_subscription | unit/config | pending |
| Envelope + headers + session | 14-03 Task 2 | CCS-03, D-05/06 | exact envelope/headers, no x-project-slug, credential-scoped session, positive continuation, no-ID non-stable | cargo test --test command_code_translate command_code_translate_session; ... _envelope; ... _headers | unit | pending |
| Tool history | 14-03 Task 3 | CCS-04, D-07 | adjacent units, missing error-text, orphan carrier, image carrier, ordering, duplicate/unsupported reject, positive subagent | cargo test --test command_code_translate command_code_translate_tools | unit | pending |
| Response machine | 14-04 Task 1 | CCS-05/06, D-08 | pinned terminal grammar, usage precision (integers only), typed unknown, empty/null fail | cargo test --test command_code_translate command_code_translate_machine | unit | pending |
| Streaming/unary relay | 14-04 Task 2 | CCS-05, D-08 | incremental relay, unary twin, one terminal, mid-body error, EOF fail | cargo test --lib command_code_tracer_response | real gateway | pending |
| Byte boundaries | 14-04 Task 3 | CCS-06/08, D-09 | per-record/residual/semantic/tool/argument caps below/at/above; UTF-8 split; deadline; no repair | cargo test --test command_code_translate command_code_bounds; --test command_code_conformance command_code_bounds | byte-split + gateway | pending |
...
- [ ] Dated protocol-evidence ledger resolves proprietary terminal grammar and exact supported field/model facts before fixtures encode assumptions. (14-02 Task 0 creates it as the Wave 0 gate; execution must complete it before 14-02 Task 1 RED fixtures.)
