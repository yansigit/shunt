...
provides:
  ...
  - "Real-router loopback conformance harness with synthetic env key, mock Chat upstream, and six tracer fixtures"
...
key-decisions:
  ...
  - "The framing [DONE] defers the authoritative terminal to EOF handling so residual frames after [DONE] can never emit success-then-error."
...
coverage:
  ...
  - id: D2
    ...
    human_judgment: false
  - id: D3
    description: "Send policy: RetrySafety::ConnectOnly with a redirect(Policy::none()) client; exactly one upstream request per generation POST, zero redirect-target requests"
    requirement: CHAT-05
    verification:
      - kind: integration
...
- **Duration:** ~180 min across two executor sessions (red-green work continued by a second executor)
- **Started:** 2026-09-07
- **Completed:** 2026-09-07
...
- All gates green through the production-isolation wrapper: full workspace 27 suites, 2709 passed, 0 failed; cargo fmt --check clean; clippy --all-targets --all-features -D warnings clean.

## Test Counts and Evidence

- openai_chat_conformance: 6 passed (3 unary + 3 streaming), run with --test-threads=1 under the OPENAI_CHAT_ENV_LOCK.
...
- The framing [DONE] only closes a provider-declared success; the authoritative terminal is deferred to EOF handling.
