...
Locale prose requires review, but file-presence/parity and the underlying behavior are automated or source-checkable. The WebSocket map has no dedicated `hot_reload_snapshot` integration test; the full suite covers per-turn parity, replacement/disconnect cancellation, and no-mid-stream hopping, while reload isolation remains a manual review item.
...
**Approval:** validated with warning noted above
...
- No implementation files were modified and no new test file was required.
- `hot_reload_snapshot` remains an explicit manual-only warning because no dedicated test exists; existing WebSocket lifecycle tests passed.
