...
Added a full exact model/effort positive and negative matrix, same-project cross-account cache isolation, same-account multi-project isolation, request-local legacy refresh rotation coverage, and real-router rejected-tuple evidence. Native lifetime fixtures prove injected resolution once per request, isolated production-default resolver counts (owned by each resolver instance), distinct account/project/token observations, prompt cancellation release of held streaming ownership, and a true in-flight A→B credential swap where request A remains on tuple A while request B observes tuple B. Eight focused library tests are selected by `antigravity_native_affinity`.
