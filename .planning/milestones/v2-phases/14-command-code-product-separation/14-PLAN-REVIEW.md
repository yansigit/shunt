...
### Subsequent independent review (2026-09-08)

Later source-provenance correction before 14-03: root inspected the complete effort table and git blame, proving its three reporter-only entries are Luna, Gemini 3.7 Flash and vision-exp, not three unspecified experimental entries. The prior research/planning mistakenly admitted two of them. The ledger, plan 03, plan 06 and COVERAGE now explicitly exclude all three, consistent with D-05 and the declared reporter-only opt-out. Ten source rows remain admitted; this is a correction before any model admission table is implemented, not a removal of an existing shipped model. No additional independent reviewer pass is claimed for this correction.

The restored host controls dispatched opencode-go/glm-5.3-flash with explicit high reasoning. The checker cleared 14-01 for execution and returned two blockers for later plans: ambiguous integration-versus-lib fixture ownership, and insufficient explicit guide-locale ownership in Task 2. Root then assigned every subscription wire filter exclusively to the crate-local router module, corrected the artifact paths, and explicitly assigned all four guide pages and i18n to the docs task. Public config fixtures remain in the integration target and execute in the workspace gate. Warnings corrected: 14-01 truth now names non-admitted auth pairings without claiming an unavailable subscription variant; both missing Japanese/Chinese subscription-env checks were added to the docs acceptance contract; read-only retry.rs was removed from the edit list. The reviewer confirmed security, requirement coverage and dependency graph. These revisions are root-applied; a second independent pass is not claimed.

The executor then prepared 14-01 using GLM/high but had not edited or run Cargo. Send/follow-up controls disappeared before release, so root interrupted it and implemented the independently cleared 14-01 inline, avoiding competing writers. This is a host control limitation, not evidence of a model quota failure; no fallback model was required.
