...
fn delta(machine: &mut OpenAiChatSseMachine, calls: Value) -> Result<(), String> {
    machine
        .process_chunk_checked(&json!({"choices":[{"delta":{"tool_calls":calls}}]}))
        .map(|_| ())
...
fn finish(mut machine: OpenAiChatSseMachine) -> Result<Value, String> {
    machine
        .process_chunk_checked(&json!({"choices":[{"delta":{},"finish_reason":"tool_calls"}]}))
        .map_err(|e| e.to_string())?;
    machine
...
fn assembly_identity_deferral_and_single_boundary_parse() {
    ...
    delta(
        &mut machine,
        json!([{"index":7,"function":{"arguments":"{\"v\":"}}]),
    )
    .unwrap();
    delta(
        &mut machine,
        json!([{"index":7,"id":"a","type":"function","function":{"name":"f","arguments":"1}"}}]),
    )
    .unwrap();
    assert_eq!(
        finish(machine).unwrap()["content"],
        json!([{"type":"tool_use","id":"a","name":"f","input":{"v":1}}])
    );
}
...
fn assembly_interleave_preserves_first_arrival_order() {
    ...
    for (index, id, args) in [
        ...
        delta(
            &mut machine,
            json!([{"index":index,"id":id,"function":{"name":"f","arguments":args}}]),
        )
        .unwrap();
    }
...
fn assembly_identity_conflict() {
    for changed in [
        ...
        .unwrap();
        assert!(
            delta(&mut machine, json!([changed])).is_err(),
            "accepted {changed}"
        );
    }
}
...
fn assembly_malformed_fields_fail_closed() {
    for call in [
        ...
        let mut machine = OpenAiChatSseMachine::new_for_upstream("m");
        assert!(
            delta(&mut machine, json!([call])).is_err(),
            "accepted {call}"
        );
    }
}
...
fn count_probe(count: usize) -> Result<Value, String> {
    ...
    for index in 0..count {
        delta(
            &mut machine,
            json!([{"index":index,"id":format!("call_{index}"),"function":{"name":"f","arguments":"{}"}}]),
        )?;
    }
    finish(machine)
}
...
#[test]
fn bound_tool_count_minus_one() {
    assert_eq!(
        count_probe(127).unwrap()["content"]
            .as_array()
            .unwrap()
            .len(),
        127
...
#[test]
fn bound_tool_count_at() {
    assert_eq!(
        count_probe(128).unwrap()["content"]
            .as_array()
            .unwrap()
            .len(),
        128
...
#[test]
fn bound_tool_count_plus_one() {
    assert!(count_probe(129).is_err());
}
...
fn args_probe(size: usize) -> Result<Value, String> {
    ...
    delta(
        &mut machine,
        json!([{"index":0,"id":"a","function":{"name":"f","arguments":&args[..split]}}]),
    )?;
    delta(
        &mut machine,
        json!([{"index":0,"function":{"arguments":&args[split..]}}]),
    )?;
    finish(machine)
}
#[test]
fn bound_args_minus_one() {
    assert_eq!(
        args_probe(1024 * 1024 - 1).unwrap()["content"][0]["input"]["v"]
            .as_str()
            .unwrap()
            .len(),
        1024 * 1024 - 9
...
fn bound_args_at() {
    assert_eq!(
        args_probe(1024 * 1024).unwrap()["content"][0]["input"]["v"]
            .as_str()
            .unwrap()
            .len(),
        1024 * 1024 - 8
...
#[test]
fn bound_args_plus_one() {
    assert!(args_probe(1024 * 1024 + 1).is_err());
}
