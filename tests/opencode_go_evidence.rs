...
const IDS: [&str; 4] = ["glm-5.3-flash", "omen-alpha", "muse-spark-1.3-contributor", "deepseek-v4-flash"];
...
fn validate(v: &Value) -> Result<(), String> {
    ...
    if rows.len() != IDS.len() { return Err("candidate count".into()); }
    for (row, id) in rows.iter().zip(IDS) {
        ...
        for key in ["wire", "headers", "context", "modalities", "effort", "tools",
            ...
            if row.get(key).is_none() || row[key].is_null() { return Err(format!("missing {id}/{key}")); }
        }
        if row["provenance"]["class"] != "source"
            || row["provenance"]["revision"] != REV
            || row["provenance"]["inspected"] != "2026-09-08"
            || row["capture"] != "none"
            || row["live"] != "none" { return Err("false provenance".into()); }
        if ["omen-alpha", "muse-spark-1.3-contributor"].contains(&id)
            && row["wire"] != "unknown" { return Err("family inference".into()); }
    }
    if v["admitted"] != json!([]) { return Err("admission must remain empty".into()); }
    let rejected = v["rejected"].as_array().ok_or("missing rejected")?;
    for reason in ["failed-evidence", "unknown-fields", "wrong-wire", "family-inference",
                   "unsupported-effort", "wrong-terminal", "unverified-live"] {
        if !rejected.iter().any(|r| r["reason"] == reason && r["disposition"].as_str().is_some_and(|s| !s.is_empty())) {
...
fn ledger() -> Value {
    ...
    assert!(std::path::Path::new(&path).is_file(), "required four-candidate evidence ledger has not been authored");
    let text = fs::read_to_string(path).expect("ledger file exists");
    let (_, rest) = text.split_once("```opencode-go-ledger\n").expect("ledger fence");
...
fn opencode_go_ledger() {
    ...
    let path = format!("{}/{PHASE}/15-EDGE-COVERAGE.json", env!("CARGO_MANIFEST_DIR"));
    let edges: Value = serde_json::from_str(&fs::read_to_string(path).unwrap()).unwrap();
    let rows = edges["items"].as_array().unwrap();
    assert_eq!(rows.len(), 8);
    let unresolved: Vec<_> = rows.iter().filter(|r| r["status"] == "unresolved").collect();
