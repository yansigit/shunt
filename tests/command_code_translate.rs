...
#[path = "command_code_translate/response.rs"]
mod response;
...
fn command_code_translate_machine_reasoning_tools_and_cache_usage() {
    ...
    let reasoning =
        machine.process_record_checked(&json!({"type":"reasoning-delta","text":"think"}));
    assert!(
        reasoning.is_ok(),
        "source-derived reasoning delta must be supported"
    );
    machine
        .process_record_checked(&json!({"type":"text-delta","text":"hello"}))
        .unwrap();
