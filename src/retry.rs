    ...
    Fut: std::future::Future<Output = Result<reqwest::Response, E>>,
{
    ...
    loop {
        ...
        // This loop ends when `.send()` returns response headers. No response
        // body or provider event has been exposed yet, so semantic commitment
        // cannot occur inside it; operation safety and retry budget are the
        // complete reissue boundary here.
