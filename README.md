...
**Inbound Codex endpoint.** shunt can also run the other direction: an opt-in `[server.codex_endpoint]` table registers raw OpenAI Responses HTTP/SSE and WebSocket transports (`/responses`, `/v1/responses`, `/backend-api/codex/responses`) so the **Codex CLI itself** can point its `base_url` at shunt and be load-balanced across the same ChatGPT/Codex OAuth account pool. A unique exact `[models.upstream_model]` or `[[routes]]` declaration may select one compatible Responses-native provider; prefix-only, non-exact, and unmatched models use the pinned `server.codex_endpoint.provider`, while exact ambiguous, translated, or incompatible targets reject before dispatch. HTTP remains a byte-for-byte relay; WebSocket upgrades authenticate before `101`, complete `generate: false` warmups locally, and stream bounded Responses events with replacement/disconnect cancellation. Provider-aware credentials and safe header filtering remain enforced, and no provider hop occurs after output begins. It also accepts the CLI's two analytics paths as privacy-preserving discard sinks, recording only sanitized event-name counters and never forwarding the payload upstream. It is off by default; absent the table, none of those routes are registered. See the [how-to](https://shunt.dev/guides/inbound-codex-endpoint/) and [M11 behavior specification](docs/m11-inbound-codex-endpoint.md).
...
The same opt-in surface registers HTTP-only `POST /v1/responses/compact`. It forwards the body and opaque continuation fields byte-for-byte through the same authentication, request limits, native route decision, and account pool to a verified native compact backend (ChatGPT/Codex or the canonical OpenAI API). Unsupported, ambiguous, translated, or malformed targets fail before network dispatch. shunt does not decrypt continuation state, synthesize summaries, or store request history.
