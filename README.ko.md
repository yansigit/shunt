...
**인바운드 Codex 엔드포인트.** shunt는 반대 방향으로도 동작할 수 있습니다. 선택 기능인 `[server.codex_endpoint]` 테이블은 raw OpenAI Responses HTTP/SSE 및 WebSocket 전송(`/responses`, `/v1/responses`, `/backend-api/codex/responses`)을 등록하므로, **Codex CLI 자체**가 `base_url`을 shunt로 지정해 같은 ChatGPT/Codex OAuth 계정 풀에서 부하 분산을 받을 수 있습니다. 고유한 정확한 `[models.upstream_model]` 또는 `[[routes]]` 선언은 호환되는 Responses-native 프로바이더 하나를 선택할 수 있고, 프리픽스 전용·비정확·불일치 모델은 고정된 `server.codex_endpoint.provider`로 폴백합니다. 정확하지만 모호하거나 변환 전용/비-Responses 또는 모델 재작성 선언은 디스패치 전에 거부됩니다. HTTP는 바이트 단위 릴레이를 유지하고 WebSocket은 제한된 이벤트를 스트리밍하며, 출력 시작 후 프로바이더 이동은 없습니다. 기본적으로 꺼져 있으며, 테이블이 없으면 해당 라우트는 등록되지 않습니다. [사용 방법](https://shunt.dev/ko/guides/inbound-codex-endpoint/)과 [M11 동작 사양](docs/m11-inbound-codex-endpoint.md)을 참고하세요.
