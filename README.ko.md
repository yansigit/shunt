...
**인바운드 Codex 엔드포인트.** shunt는 반대 방향으로도 동작할 수 있습니다. 선택 기능인 `[server.codex_endpoint]` 테이블은 raw OpenAI Responses HTTP/SSE 및 WebSocket 전송(`/responses`, `/v1/responses`, `/backend-api/codex/responses`)을 등록하므로, **Codex CLI 자체**가 `base_url`을 shunt로 지정해 같은 ChatGPT/Codex OAuth 계정 풀에서 부하 분산을 받을 수 있습니다. HTTP는 바이트 단위 릴레이를 유지하고, WebSocket은 `101` 전에 인증하며 `generate: false` 워밍업을 로컬에서 완료하고 교체/연결 종료 취소가 적용된 제한된 Responses 이벤트를 스트리밍합니다. 또한 CLI의 두 애널리틱스 경로를 프라이버시를 지키는 폐기 싱크로 받아들입니다. 기본적으로 꺼져 있으며, 테이블이 없으면 해당 라우트는 등록되지 않습니다. [사용 방법](https://shunt.dev/ko/guides/inbound-codex-endpoint/)과 [M11 동작 사양](docs/m11-inbound-codex-endpoint.md)을 참고하세요.
