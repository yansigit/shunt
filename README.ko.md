...
네이티브 Antigravity는 표준 HTTPS 루트 `daily-cloudcode-pa.googleapis.com`과 `cloudcode-pa.googleapis.com`(daily로 정규화)을 사용하며 리디렉션의 정확한 오리진을 검사합니다. 최신 `fetchAvailableModels` 증거가 정확한 모델/effort를 허용해야 추론하며, 모델 추측이나 effort 조정은 하지 않습니다. 두 클라이언트 모드 모두 `streamGenerateContent?alt=sse`를 사용합니다. 범위가 지정된 `call_antigravity_v2_` ID와 도구 기록 순서를 보존하세요. 세션은 계정과 정규화된 첫 메시지에서 파생되므로 같은 계정의 동일한 첫 메시지는 세션을 공유합니다. 키 없는 태그는 컨텍스트 검사이며 업스트림 출처의 암호학적 증명이 아닙니다. 카탈로그 조회는 리디렉션을 거부하며, 구성된 엔드포인트의 증거만 추론 허용에 사용합니다.

최초 `401`에는 같은 프로젝트/세션/본문으로 같은 계정의 메모리 내 갱신과 재전송을 한 번 허용하며 새 자격 증명 파일 기록은 없습니다. 출력, 도구, 파서/본문 오류, 전송 여부가 불확실한 실패 뒤에는 재전송하지 않고, 취소하면 업스트림 작업과 수용 슬롯이 해제됩니다. 증거는 합성 데이터 기반 격리 테스트이며 실시간 제공을 보장하지 않습니다. Google AI Studio Web은 제외됩니다.
...
동일한 선택 기능은 레거시 HTTP 전용 `POST /v1/responses/compact`도 등록합니다. 본문과 불투명한 연속 상태는 정식 OpenAI API 키 백엔드에만 바이트 단위 그대로 전달됩니다. ChatGPT/Codex 백엔드는 더 이상 이 별도 엔드포인트를 제공하지 않으므로 ChatGPT OAuth 대상은 자격 증명 확인이나 네트워크 디스패치 전에 로컬에서 실패합니다. 현재 Codex의 원격 compaction V2는 대신 일반 Responses 스트림에 마지막 `compaction_trigger`를 보내며, 네이티브 경로는 이를 변경 없이 전달합니다. 지원되지 않거나 모호하거나 변환이 필요한 대상 및 잘못된 요청도 네트워크 디스패치 전에 거부됩니다. shunt는 연속 상태를 복호화하거나 요약을 합성하거나 요청 기록을 저장하지 않습니다.
...
JSON과 SSE는 추론·텍스트 순서 및 사용량을 유지하며 취소하면 업스트림 턴과 게이트웨이 용량이 해제됩니다.

잘못된 Run 프레임이나 도구 인수는 명시적으로 오류를 반환합니다. Run은 자동 재시도하지 않으며 전송 전 연결 실패가 확인된 경우에만 설정된 폴백으로 이동할 수 있습니다. 수락 후 오류, 부분 출력 및 도구 호출은 재전송을 유발하지 않습니다. 이는 격리된 적합성 검증이며 실제 모델 가용성을 보장하지 않습니다.
...
인바운드 Codex 경로에서는 `[[server.codex_endpoint.routes]]`의 대소문자 구분 정확한 바이트 일치가 우선합니다. 불일치하면 `[1m]` 정규화를 포함한 기존 전역 해석과 고정 프로바이더로 폴백합니다.
